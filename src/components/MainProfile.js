...
function MainProfile() {
    ...
    useEffect(() => {
        fetch(`http://${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/${process.env.REACT_APP_PREFIX}/api/get_user_levle/${next}`)
          .then((response) => {
            if (!response.ok) {
      ...
      useEffect(() => {
        fetch(`http://${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/${process.env.SERVER_PREFIX}/api/get_user_count`)
          .then((response) => {
            if (!response.ok) {
              throw new Error("Network response was not ok");
            }
