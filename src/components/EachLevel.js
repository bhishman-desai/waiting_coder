...
function EachLevel({setIdCollectionValue, setId}) {
    ...
    useEffect(() => {
      fetch(`http://${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/${process.env.REACT_APP_PREFIX}/get_each_user_levle/${localStorage.getItem('idEachProfile')}`)
        .then((response) => {
          if (!response.ok) {
    ...
    useEffect(() => {
      fetch(`http://${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/${process.env.SERVER_PREFIX}/name_specifics/${localStorage.getItem('idEachProfile')}`)
        .then((response) => {
          if (!response.ok) {
            throw new Error('Network response was not ok');
          }
    ...
    useEffect(() => {
      fetch(`http://${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/${process.env.SERVER_PREFIX}/api/validation_specific/${localStorage.getItem('idEachProfile')}/${idCollection}`)
        .then((response) => {
          if (!response.ok) {
            throw new Error('Network response was not ok');
          }
