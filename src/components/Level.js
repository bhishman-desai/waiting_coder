...
function Level({ selectedValues, setSelectedValues, setSelectedValuesTools, setSelectedValuesProject}) {
  ...
  useEffect(() => {
    fetch(`api/api/get_languages/${selectLocalStor}`)
      .then((response) => {
        if (!response.ok) {
  ...
  useEffect(() => {
    fetch(`/api/get_tools/${selectLocalStor}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
  ...
  useEffect(() => {
    fetch(`/api/get_porject/${selectLocalStor}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
...
const [countExp, setCountExp] = useState(0);
  ...
  useEffect(() => {
    var data2;
    fetch(`/api/get_count_exp/${userId}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
