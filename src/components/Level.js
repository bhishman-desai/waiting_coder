...
function Level({ selectedValues, setSelectedValues, setSelectedValuesTools, setSelectedValuesProject}) {
  ...
  useEffect(() => {
    fetch(`http://${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/${process.env.REACT_APP_PREFIX}/api/get_languages/${selectLocalStor}`)
      .then((response) => {
        if (!response.ok) {
  ...
  useEffect(() => {
    fetch(`http://${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/${process.env.SERVER_PREFIX}/api/get_tools/${selectLocalStor}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
  ...
  useEffect(() => {
    fetch(`http://${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/${process.env.SERVER_PREFIX}/api/get_porject/${selectLocalStor}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
  ...
  useEffect(() => {
    fetch(`http://${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/${process.env.SERVER_PREFIX}/name_specifics/${userId}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
  ...
  useEffect(() => {
    fetch(`http://${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/${process.env.SERVER_PREFIX}/api/get_levels/${userId}`)
      .then((response) => {
        if (!response.ok) {
...
const [countExp, setCountExp] = useState(0);
  ...
  useEffect(() => {
    var data2;
    fetch(`http://${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/${process.env.SERVER_PREFIX}/api/get_count_exp/${userId}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
      ...
      // console.log("test ok ",data.length > 0 ? data[0].exp.toString().split(".")[1] : 1);
          fetch(`http://${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/${process.env.SERVER_PREFIX}/api/update_level/${userId}`, {
            method: 'POST',
            headers: {
